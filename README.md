...
| 18 | [Tree Sort](#18-tree-sort) | Không ổn định |
...
Tree Sort là thuật toán sắp xếp dựa trên **Binary Search Tree (BST)**. Thuật toán chèn lần lượt các phần tử vào cây tìm kiếm nhị phân, sau đó thực hiện phép duyệt **In-order Traversal** để lấy các phần tử theo thứ tự tăng dần.
...
**Ưu điểm:** ý tưởng đơn giản, tận dụng tính chất của cây tìm kiếm nhị phân và dễ mở rộng khi sử dụng cây cân bằng.

**Nhược điểm:** nếu cây bị lệch (không cân bằng), hiệu năng giảm xuống `O(n²)` và cần thêm bộ nhớ để lưu cây.
